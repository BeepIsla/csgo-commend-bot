...
const config = require("./config_undocommends.json");
const colors = {
	general: "\x1b[37m",
	login: "\x1b[36m",
	loggedIn: "\x1b[33m",
...
if (process.argv[3]) {
	try {
		if (!isNaN(parseInt(process.argv[3]))) {
			config.CommendsToRemove = parseInt(process.argv[3]);
		} else {
			throw new Error("Not a number");
		}
	} catch(e) {
		console.log(colors.error + "Failed to parse CommendsToRemove via command line argument");
	};
}
...
(async () => {
	...
	console.log(colors.general + "Getting " + config.CommendsToRemove + " already used account" + (config.CommendsToRemove === 1 ? "" : "s"));
	...
	if (available.length < config.CommendsToRemove) {
		console.log(colors.general + available.length + "/" + config.accounts.length + " account" + (config.accounts.length === 1 ? "" : "s") + " available but we need " + config.CommendsToRemove + " account" + (config.CommendsToRemove === 1 ? "" : "s"));
		return;
	}

	// Get needed accounts
	var accountsToUse = available.slice(0, config.CommendsToRemove);
...
function accountHandler(account, resolve) {
	try {
		...
		acc.on("ready", async (hello) => {
			...
			acc.commend(config.AccountToCommend, (30 * 1000), false, false, false).then((response) => {
				commendsSent += 1;

				console.log(colors.success + "[" + account.username + "] Successfully remove a commend " + commendsSent + "/" + config.CommendsToRemove);
				...
				if (index >= 0) {
					...
					var index2 = config.accounts[index].commended.indexOf(config.AccountToCommend);
					config.accounts[index].commended.splice(index2, 1);
				}
...
function checkComplete(resolve) {
	...
	// Global complete
	if ((commendsSent + commendsFailed) >= config.CommendsToRemove) {
		resolve(false);
		...
		console.log(colors.general + "Successfully removed " + commendsSent + "/" + config.CommendsToRemove + " commend" + (config.CommendsToRemove === 1 ? "" : "s") + ", " + commendsFailed + " commend" + (commendsFailed === 1 ? "" : "s") + " failed");
		return;
	}
